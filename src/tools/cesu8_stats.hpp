...
Cesu8Stats validate_and_collect_stats(byte const* str, std::size_t len)
{
        ...
        Cesu8Stats stats;
        while (len > 0) {
                if (is_cu1(*str)) { // 1-byte codepoint
                        ++str;
                        --len;
                        ++stats.nr_c1;
                }
                else if (is_cu2(*str)) { // 2-byte codepoint
                        str += 2;
                        len -= 2;
                        ++stats.nr_c2;
                }
                else if (is_cu3(*str)) {
                        if (!is_sp(*str) || !is_hi_or_lo(str[1])) { // 3-byte codepoint
                                str += 3;
                                len -= 3;
                                ++stats.nr_c3;
                        }
                        else if (is_sp_hi(*str, str[1])) { // 6-byte codepoint
                                str += 6;
                                len -= 6;
                                ++stats.nr_surrogate;
                        }

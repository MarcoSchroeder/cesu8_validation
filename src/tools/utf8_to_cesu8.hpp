...
static uint16_t* cesu8_to_utf16_pairs(uint8_t* bytes, int offset)
{
        ...
        uint16_t high =
            (uint16_t)0xD800 + ((bytes[offset + 1] - 0xA0) << 6) + (bytes[offset + 2] - 0x80);

        uint16_t low =
            (uint16_t)0xDC00 + ((bytes[offset + 4] - 0xB0) << 6) + (bytes[offset + 5] - 0x80);
...
static void copy_cesu8_as_utf8_buffer(char* dest, const char* cesu8, size_t cesu8_length)
{
        size_t    dest_index = 0;
        size_t    utf8_index;
        uint16_t* pairs;
...
static void copy_utf8_as_cesu8_buffer(char* dest, const char* utf8, size_t utf8_length)
{
        int    dest_index = 0;
        size_t i;

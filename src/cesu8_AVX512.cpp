...
__m512i x_mm512_alignr_epi8(const __m512i hi, const __m512i lo, unsigned shift) {
    ...
    if (s8 == 0) {
        ...
        return _mm512_or_si512(
                _mm512_srli_epi32(L, s8 * 8),
                _mm512_slli_epi32(H, (4 - s8) * 8)
        );
    }
}
...
{
        static __m512i const lookup_table3 = _mm512_setr4_epi32(0x33333333, 0x33333333,
                                                                0x3C3C3C3C, 0x00333333);
        static __m512i const lookup_table2 = _mm512_setr4_epi32(0x14141414, 0x14141414,
                                                                0x5A9A3A3A, 0x00111111);
        static __m512i const lookup_table1_high = _mm512_setr4_epi32(0x05050505, 0x05050505,
                                                                     0x0D0D0D0D, 0x00E70D1D);
        static __m512i const lookup_table1_low = _mm512_setr4_epi32(0x0F0F1F3F, 0x0F0F0F0F,
                                                                    0x0F0F0F0F, 0x0F0FCF0F);
        ...
        while (len >= 64) {
                ...
                __m512i shifted_high_surrogates = avx512_push_last_3bytes_of_a_to_b(high_surrogates, prev_high_surrogates);
                __m512i error_flags_e = _mm512_xor_si512(shifted_high_surrogates, low_surrogates);

                // Illegal header
                has_error_header = _kor_mask64(has_error_header, _mm512_cmpeq_epi8_mask(_mm512_set1_epi8(0xF), nibbles3));
    ...
    */
        ...
        size_t offset = 0;
        if (is_ed_a_c0(str[-5], str[-4], str[-3])) {
                offset = 5;
        }
        else if (is_ed_a_c0(str[-4], str[-3], str[-2])) {
                offset = 4;
        }
        else if (is_ed_a_c0(str[-3], str[-2], str[-1])) {
                offset = 3;
        }

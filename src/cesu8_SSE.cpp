...
bool is_valid_cesu8_SSE(byte const* str, std::size_t len)
{
        ...
        static __m128i const table1_high =
            _mm_setr_epi8(CESU8_IMPLEMENTATION_LOOKUPTABLE_1HIGH_VALUES);
        static __m128i const table1_low =
            _mm_setr_epi8(CESU8_IMPLEMENTATION_LOOKUPTABLE_1LOW_VALUES);

        __m128i prev_bytes =
            _mm_setzero_si128(); // Zero values will be interpeted as preceding ascii in first loop iteration
        ...
        while (len >= 16) {
                ...
                __m128i nibbles1_high =
                    _mm_and_si128(_mm_srli_epi16(bytes1, 4), _mm_set1_epi8(0x0F));
                ...
                __m128i shifted_high_surrogates =
                    _mm_alignr_epi8(high_surrogates, prev_high_surrogates, 13);

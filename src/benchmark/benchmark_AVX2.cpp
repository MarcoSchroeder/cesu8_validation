...
static void BM_ascii_data_small(benchmark::State& state)
{
        ...
        for (auto _ : state) {
                for (size_t i = 0; i < ITERATIONS_SMALL; ++i) {
                        cesu8::is_valid_cesu8_AVX2(data, state.range(0));
                }
        }
        delete[] data;
...
BENCHMARK(BM_surrogate_data_small)
    ->RangeMultiplier(2)
    ->Range(SMALL_MIN, SMALL_MAX)
    ->Unit(benchmark::kMicrosecond);
